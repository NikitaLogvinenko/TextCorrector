...
// Если в таблице становится слишком много слов, то сначала таблица увеличивается в несколько раз, чтобы уменьшить число коллизий (для более быстрого поиска в дальнейшем)
// Но если увеличения не произошло (не выделилась память), то слово добавиться в старую таблицу
...
// Если не выделилась память под узел для нового слово - вернёт EXIT_MEMORY_FAILURE, а таблица останется прежней
