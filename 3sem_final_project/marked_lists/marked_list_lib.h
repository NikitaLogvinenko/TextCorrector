...
// Память под слово должна быть выделена динамически. Если не удаётся вставить слово или такое уже записано в списке - память очищается, т.к. слово нигде не нужно вне списка
// Счётчик начать с counter (т.к. можем загружать модель из файла, то начальный counter не обязательно 1)
// Если при добавлении counter произойдёт выход за верхнюю границу unsigned, то просто устанавливаем значение counter в UINT_MAX - максимально допустимое
// UINT_MAX определён в <limits.h> и равен 65535 (для 2 байт) или 4 294 967 295 (для 4 байт).
